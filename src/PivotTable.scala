...
spark.sql("select level,date_format(datetime,'MMMM') as month,cast(date_format(datetime,'M') as int) as monthnum from logging_table").groupBy("level").pivot("monthnum").count.show(100)
